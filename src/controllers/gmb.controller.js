...
import {
  readGmbReviewMonthlyPayloadCached,
  runGmbReviewMonthlyRebuildJob,
  getClientMonthlyHistoryOrBuild,
  filterRowsByRegion,
  enrichRowForLiveMonth,
  currentMonthId,
} from '../services/gmbReviewMonthly.service.js';
...
/**
 * GET /api/gmb-review-monthly?region=es
 * Paying + keywords + GBP (`services-keywords.json`) — reviews per calendar month.
 */
export async function getGmbReviewMonthlySnapshot(req, res) {
  try {
    res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate');
    res.type('application/json');
    const region = String(req.query.region || '').trim().toLowerCase();
    const payload = await readGmbReviewMonthlyPayloadCached();
    const rows = filterRowsByRegion(payload.rows || [], payload, region).map((r) =>
      enrichRowForLiveMonth(r),
    );
    const { sourcePath, ...body } = payload;
    const summaryRoot = body.summary && typeof body.summary === 'object' ? body.summary : {};
    const byRegion =
      summaryRoot.withKeywordsByRegion && typeof summaryRoot.withKeywordsByRegion === 'object'
        ? summaryRoot.withKeywordsByRegion
        : {};
    return res.json({
      ...body,
      ok: true,
      ...
      summary: {
        ...
        withKeywordsPool: byRegion?.[region] ?? rows.length,
        withReviewsThisMonth: rows.filter((r) => (r.reviewsThisMonth ?? 0) > 0).length,
        noReviewsThisMonth: rows.filter((r) => (r.reviewsThisMonth ?? 0) === 0).length,
      },
      _meta: sourcePath ? { source: 'ranking_server', path: sourcePath } : { source: 'empty' },
    });
  } catch (err) {
    console.error('[gmb getGmbReviewMonthlySnapshot]', err);
    return res.status(500).json({ ok: false, error: err.message || 'read_failed' });
  }
}

/**
 * GET /api/gmb-review-monthly/client?clientId=...&region=es
 */
export async function getGmbReviewMonthlyClientHistory(req, res) {
  try {
    res.setHeader('Cache-Control', 'no-store');
    res.type('application/json');
    const clientId = String(req.query.clientId || '').trim();
    ...
    const { history, source } = await getClientMonthlyHistoryOrBuild(clientId, region);
    return res.json({ ok: true, ...history, _meta: { source: source || 'snapshot' } });
  } catch (err) {
    console.error('[gmb getGmbReviewMonthlyClientHistory]', err);
    return res.status(500).json({ ok: false, error: err.message || 'history_failed' });
  }
}

/**
 * POST /api/gmb-review-monthly/rebuild
 */
export async function rebuildGmbReviewMonthlySnapshot(req, res) {
  try {
    req.socket?.setTimeout?.(0);
    res.setTimeout?.(0);
    const out = await runGmbReviewMonthlyRebuildJob();
    if (!out?.ok) {
      return res.status(500).json({
        ok: false,
        error: out?.error || 'rebuild_failed',
    ...
    console.error('[gmb rebuildGmbReviewMonthlySnapshot]', err);
    return res.status(500).json({ ok: false, error: err.message || 'rebuild_failed' });
  }
}
