...
async function captureCheckUrlScreenshotZenrows({ checkUrl, rank, businessName, keyword = '' }) {
  ...
  console.log(`[DataForSEO/Capture] Calling Scrapfly Screenshot API...`);
  ...
  try {
    ...
    console.error(`[DataForSEO/Capture] Scrapfly Screenshot API error:`, err?.message || err);
    throw err;
  }
  ...
  // Step 2: Use DataForSEO rank for position (GMB verified by DataForSEO)
  const realRank = rank;

  console.log(
    `[DataForSEO/Capture] Using DataForSEO rank #${rank} (slot ${slotOnPage}) for overlay`
  );

  // Step 3: Add red marker overlay using Sharp
  // For FULL PAGE screenshot - Google Local Finder layout at 1440px width
  // Header + search bar + tabs = ~170px
  // Each GMB card is ~160px tall on full page
  ...
  console.log(`[DataForSEO/Capture] Saved ${filename} (rank #${realRank})`);
