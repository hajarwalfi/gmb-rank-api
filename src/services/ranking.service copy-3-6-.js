...
export async function captureGoogleSearchLocalScreenshot({
  ...
  // Step 1: Use Scrapfly Screenshot API - FULL PAGE capture for all GMB cards
  ...
  screenshotUrl.searchParams.set('capture', 'fullpage'); // Full page screenshot
  screenshotUrl.searchParams.set('rendering_wait', '5000');
  screenshotUrl.searchParams.set('wait_for_selector', '.rllt__details,.VkpGBb,[data-cid]');
  screenshotUrl.searchParams.set('auto_scroll', 'true'); // Scroll to load all GMB cards
  screenshotUrl.searchParams.set('country', 'us');
  screenshotUrl.searchParams.set('proxy_pool', 'public_residential_pool');
  screenshotUrl.searchParams.set('asp', 'true');
  screenshotUrl.searchParams.set('cache', 'false');
  screenshotUrl.searchParams.set('timeout', '90000');
  screenshotUrl.searchParams.set('options', 'block_banners'); // Remove cookie banners

  console.log(`[GoogleLclCapture] Calling Scrapfly Screenshot API...`);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Scrapfly Screenshot API failed: ${response.status} - ${errorText.slice(0, 300)}`);
    }
    ...
    console.error(`[GoogleLclCapture] Scrapfly Screenshot API error:`, err?.message || err);
    throw err;
  }
  ...
  console.log(`[GoogleLclCapture] Using DataForSEO rank #${r} (slot ${slotOnPage}) for overlay`);

  // Step 3: Add red marker overlay using Sharp
  // For FULL PAGE screenshot - Google Local Finder layout at 1440px width
  const cardTopOffset = 170; // Below Google header/tabs
  const cardHeight = 160; // Full GMB card height
  const cardLeftOffset = 50; // Left margin for GMB cards
  const cardWidth = 580; // GMB card width
  const borderWidth = 4;
  const padding = 8;
  const badgeSize = 36;
  const fontSize = 18;
  ...
  const estimatedRect = {
    x: cardLeftOffset,
    y: cardTopOffset + (slotOnPage - 1) * cardHeight,
    width: cardWidth,
  ...
  const overlayX = Math.max(0, estimatedRect.x - padding);
  const overlayY = Math.max(0, estimatedRect.y - padding);
  const overlayWidth = Math.min(imgWidth - overlayX, estimatedRect.width + 2 * padding);
  const overlayHeight = Math.min(imgHeight - overlayY, estimatedRect.height + 2 * padding);

  const svgOverlay = `
    <svg width="${imgWidth}" height="${imgHeight}" xmlns="http://www.w3.org/2000/svg">
      <rect x="${overlayX}" y="${overlayY}" width="${overlayWidth}" height="${overlayHeight}" 
            fill="none" stroke="${markerColor}" stroke-width="${borderWidth}"/>
      <rect x="${overlayX}" y="${Math.max(0, overlayY - badgeSize)}" 
            width="${badgeSize}" height="${badgeSize}" fill="${markerColor}"/>
      <text x="${overlayX + badgeSize / 2}" y="${Math.max(badgeSize / 2, overlayY - badgeSize / 2 + 6)}" 
            font-family="Arial, sans-serif" font-size="${fontSize}" font-weight="bold" 
            fill="white" text-anchor="middle" dominant-baseline="middle">#${realRank}</text>
    </svg>
  `;
  ...
  console.log(`[GoogleLclCapture] Saved ${fn} (rank #${realRank})`);
