...
const server = app.listen(PORT, async () => {
  ...
  const hasZen = !!((process.env.ZENROWS_API_KEY || '').trim());
  const hasDfs =
  ...
  const zr = ((process.env.ZENROWS_PROXY_REGION || '').trim() || '(unset)');
  console.log(
    `ZenRows (udm=1 screenshots): ${hasZen ? 'set — capture uses cloud browser' : 'NOT set — capture uses local Playwright + PLAYWRIGHT_PROXY only'}`
  );
  if (hasZen && !(process.env.ZENROWS_PROXY_REGION || '').trim()) {
    console.warn(
      'ZENROWS_PROXY_REGION not set — exits may be EU/AP; set na (North America) for SerpAPI gl=us / Texas-style parity.'
    );
  } else if (hasZen) {
    console.log(`ZenRows proxy_region: ${zr}`);
  }
  console.log(
